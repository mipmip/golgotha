...
func TestSaveLoadRoundTrip(t *testing.T) {
	...
	// File lives at <dir>/huphop/github.json.
	if _, err := os.Stat(filepath.Join(dir, "huphop", "github.json")); err != nil {
...
func TestSaveAtomicNoTempLeftBehind(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(dir, "huphop"))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSaveMkdirError(t *testing.T) {
	dir := t.TempDir()
	// Make the XDG cache root a regular file so MkdirAll(<file>/huphop) fails.
...
func TestDirFallbackToHome(t *testing.T) {
	...
	if d != filepath.Join(home, ".cache", "huphop") {
		t.Fatalf("dir = %q", d)
	}
}
...
func TestPath(t *testing.T) {
	...
	if p != filepath.Join(dir, "huphop", "github.json") {
		t.Fatalf("path = %q", p)
	}
}
